...
3. On the user's **next** message, a small yes/no check decides whether that message
   is an explicit go-ahead. If so, the **stored** call is executed exactly as the user
   saw it (not regenerated by the model, which would never match for something like
   file contents). Any other message discards the pending action.

A destructive action therefore cannot run within one request, even if the model
ignores its instructions, and it only ever runs with the arguments the user approved.
